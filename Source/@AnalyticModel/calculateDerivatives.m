function calculateDerivatives(this, opts)
% Prepares a constructed AnalyticModel for use with the rest of kroneckerbio. 
%   Generates symbolic expressions and requires the symbolic toolbox.
% 
%   Inputs:
%   this [ AnalyticModel ]
...
opts_.EvaluateExternalFunctions = true; % needed for calls to power() and other functions
...
xu_full_names = m.xu_full_names;

unique_x_names = m.unique_x_names;
unique_u_names = m.unique_u_names;
...
dv = vec([this.Compartments.Dimension]);
v = vec({this.Compartments.Size});

s = vec([this.Seeds.Value]);

k = vec([this.Parameters.Value]);

u = vec([this.Inputs.DefaultValue]);

x0 = vec({this.States.InitialValue});

r = vec({this.Reactions.Rate});

z = vec({this.Rules.Expression});

y = vec({this.Outputs.Expression});
...
S_entries = zeros(0,3);
for ir = 1:nr
    reaction = this.Reactions(ir);
    for j = 1:numel(reaction.Reactants)
        species = reaction.Reactants{j};
        ind = lookupmember(species, xu_full_names);
        S_entries = [S_entries; ind, ir, -1];
    end
    for j = 1:numel(reaction.Products)
        species = reaction.Products{j};
        ind = lookupmember(species, xu_full_names);
        S_entries = [S_entries; ind, ir, 1];
    end
end
