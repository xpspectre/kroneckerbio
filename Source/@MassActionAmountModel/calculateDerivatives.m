function calculateDerivatives(this)
%Update the mathematical components of a MassActionAmountModel
%
...
for ir = 1:nr
    sorted_reactions{ir} = {sort(this.Reactions(ir).Reactants), sort(this.Reactions(ir).Products), this.Reactions(ir).Parameter, this.Reactions(ir).Name};
end
...
% Dimensions
m.dv = vec([this.Compartments.Dimension]);
...
for iv = 1:nv
    if isnumeric(this.Compartments(iv).Size)
        ...
        bValues(nbEntries) = this.Compartments(iv).Size;
    elseif iscell(this.Compartments(iv).Size)
        % Handle compartment size given as a list of strings
        nExpr = size(this.Compartments(iv).Size,1);
        for iExpr = 1:nExpr
            % Find states that match the expression
            match = find(strcmp(this.Compartments(iv).Size{iExpr,1}, x_full_names));
            nAdd = numel(match);
            ...
            B1Values(nB1Entries-nAdd+1:nB1Entries) = this.Compartments(iv).Size{iExpr,2};
            
            % Find inputs that match the string
            match = find(strcmp(this.Compartments(iv).Size{iExpr,1}, u_full_names));
            nAdd = numel(match);
            ...
            B2Values(nB2Entries-nAdd+1:nB2Entries) = this.Compartments(iv).Size{iExpr,2};
            
            % Find empty expressions, which are constants
            if isempty(this.Compartments(iv).Size{iExpr,1})
                nbEntries = nbEntries + 1;
                ...
                bEntries(nbEntries,1) = iv;
                bEntries(nbEntries,2) = 1;
                bValues(nbEntries) = this.Compartments(iv).Size{iExpr,2};
            end
        end
    else
...
%% Process seeds
m.s = vec([this.Seeds.Value]);

%% Process states
% Set up map from seeds to initial conditions
state_seed_values = vec({this.States.InitialValue});
...
dx0ds_val = sparse(dx0dsEntries(:,1), dx0dsEntries(:,2), dx0dsValues, nx, ns);
...
m.u = cat(1, this.Inputs.DefaultValue);
...
for iy = 1:ny
    nExpr = size(this.Outputs(iy).Expression,1);
    for iExpr = 1:nExpr
        % Find states that match the string
        match = find(strcmp(this.Outputs(iy).Expression{iExpr,1}, x_full_names));
        nAdd = numel(match);
        ...
        C1Values(nC1Entries-nAdd+1:nC1Entries) = this.Outputs(iy).Expression{iExpr,2};
        
        % Find inputs that match the string
        match = find(strcmp(this.Outputs(iy).Expression{iExpr,1}, u_full_names));
        nAdd = numel(match);
        ...
        C2Values(nC2Entries-nAdd+1:nC2Entries) = this.Outputs(iy).Expression{iExpr,2};

        % Find empty expressions, which are constants
        if isempty(this.Outputs(iy).Expression{iExpr,1})
            ...
            cValues(ncEntries) = this.Outputs(iy).Expression{iExpr,2};
        end
    end
end
...
for ir = 1:nr
    ...
    n_reac = numel(this.Reactions(ir).Reactants);
    n_prod = numel(this.Reactions(ir).Products);
    ...
    for i_reac = 1:n_reac
        if isempty(this.Reactions(ir).Reactants{i_reac})
            reactantsExist(i_reac) = false;
        else
            reactantsExist(i_reac) = true;
            reactantsAreStates(i_reac) = true;
            temp = find(strcmp(this.Reactions(ir).Reactants(i_reac), x_full_names), 1);
            if ~isempty(temp)
                ...
                reactants(i_reac) = find(strcmp(this.Reactions(ir).Reactants(i_reac), u_full_names), 1);
    ...
    for i_prod = 1:n_prod
        temp = find(strcmp(this.Reactions(ir).Products{i_prod}, x_full_names), 1);
        if ~isempty(temp)
    ...
    % Extract parameter
    parameter = find(strcmp(this.Reactions(ir).Parameter{1}, k_names));
    assert(~isempty(parameter), 'KroneckerBio:FinalizeModel:MissingReactionParameter', 'Reaction %s (#%i) requires parameter %s, but no parameter by that name was found', this.Reactions(ir).Name, ir, this.Reactions(ir).Parameter{1})
    m.krInd(ir) = parameter; % Store index
    
    % The multiplier that is applied to the rate parameter
    modifier = this.Reactions(ir).Parameter{2};
...
m.d2rdkdx = d2rdkdxHidden(m.dD1dk_rk_x, m.dD2dk_rk_xx, m.dD3dk_rk_ux, m.dD4dk_rk_xu, m.dD5dk_rk_uu, m.v, m.dvdx, D2UsedColumns, D2UsedSpecies1, D2UsedSpecies2, D3UsedColumns, D3UsedSpecies1, D3UsedSpecies2, D4UsedColumns, D4UsedSpecies1, D4UsedSpecies2, D5UsedColumns, D5UsedSpecies1, D5UsedSpecies2, m.vxInd, m.vuInd, nr);
...
m.d2rdkdu = d2rdkduHidden(m.dD2dk_rk_xx, m.dD3dk_rk_ux, m.dD4dk_rk_xu, m.dD5dk_rk_uu, m.dD6dk_rk_u, m.v, m.dvdu, D2UsedColumns, D2UsedSpecies1, D2UsedSpecies2, D3UsedColumns, D3UsedSpecies1, D3UsedSpecies2, D4UsedColumns, D4UsedSpecies1, D4UsedSpecies2, D5UsedColumns, D5UsedSpecies1, D5UsedSpecies2, m.vxInd, m.vuInd, nr);
...
import Util.linearindexpermute
