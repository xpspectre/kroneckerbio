...
yNamesOld = {m.Outputs.Name};
yNamesOld(cellfun(@isempty, yNamesOld)) = [];
yNamesNew = {m.add.Outputs.Name};
yNamesNew(cellfun(@isempty, yNamesNew)) = [];

yOld = ismember(yNamesOld, name);
yNew = ismember(yNamesNew, name);
if any(yOld) % Pull output existing in finalized model
    output = m.Outputs(yOld);
    m.Outputs(yOld) = [];
    m.ny = m.ny - 1;
elseif any(yNew) % Pull output existing in m.add
    output = m.add.Outputs(yNew);
    m.add.Outputs(yNew) = [];
    m.add.ny = m.add.ny - 1;
else % Add completely new output
    error('AddEps:invalidOutput', 'Output %s for intra-individual variability not found in model.', name)
end
...
m = addOutputAnalytic(m, output.Name, errormodel);

%% Add output for F (state space solution, no eps, derivatives wrt eta will be taken)
m = addOutputAnalytic(m, ['Fi__' output.Name], output.Expression, output.ID);

%% Add new parameters
for i = 1:n
    m = addParameterAnalytic(m, eps{i}, 0);
end
