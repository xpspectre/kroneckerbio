...
elseif l == nk*nCon+ns*nCon+nq+nh
    bounds = bounds([vec(UseParams); vec(UseSeeds); cat(1,UseInputControls{:}); cat(1,UseDoseControls{:})]);
elseif l == nT
    %bounds = bounds; % do nothing
elseif l == nk && nTs == 0 && nTq == 0 && nTh == 0
    bounds = bounds(UseParams(:,1));
elseif l == nk*nCon+ns*nCon && nTq == 0 && nTh == 0
    bounds = bounds([vec(UseParams); vec(UseSeeds)]);
else
